...
import uuid
import datetime
from sqlalchemy.orm import relationship, backref # Import 'backref' for inverse relationships
...
class UserDBModel(db.Model):
    ...
    registration_date = db.Column(db.DateTime, default=datetime.datetime.utcnow, name='registration_date') # Original name 'data_cadastro'
    ...
    address_id = db.Column(db.String(36), db.ForeignKey('addresses.address_id'), nullable=True, name='id_endereco') # FK as per diagram
    ...
    address_info = relationship('AddressDBModel', backref='users_using_this_address', uselist=False) 
    ...
    buyer_profile = relationship('BuyerDBModel', back_populates='user', uselist=False, primaryjoin="UserDBModel.user_id == BuyerDBModel.buyer_id")
    artisan_profile = relationship('ArtisanDBModel', back_populates='user', uselist=False, primaryjoin="UserDBModel.user_id == ArtisanDBModel.artisan_id")
    ...
    def __init__(self, email: str, password_hash: str, status: str = 'active', 
                 user_id: str = None, registration_date: datetime.datetime = None,
        ...
        self.status = status
        self.registration_date = registration_date if registration_date else datetime.datetime.utcnow()
